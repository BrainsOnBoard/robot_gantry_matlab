...
public:
  ...
  SIMPLE_EXCEPTION_CLASS( CannotMoveToPositionException )
   ...
   */
  virtual void getPosition( GantryPosition& positionBuffer ) const = 0;
   ...
   */
  virtual const GantryPosition& getPosition() const = 0;
   ...
   */
    ...
    * @see moveToZPosition
    */
   virtual void moveToPosition( const GantryPosition& position,
    ...
    * axes remains the same. Note that the position is checked after the
    * move has been attempted, and retried 10 times, but only if the waitFlag
    * is set to true. If it is set to false, it is the users responsibility
    * to test that the move has been successful.
    *
    * @param     x                             The x position in millimeters
    * @param     waitFlag                      A boolean flag to state whether
    *                                          to wait or not.
   ...
   virtual void followSlewData( const SlewData&, SlewUpdate * = NULL ) = 0;
};
