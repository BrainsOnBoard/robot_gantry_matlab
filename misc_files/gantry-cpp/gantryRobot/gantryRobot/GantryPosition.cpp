...
GantryPosition::GantryPosition() : m_X( 0 ),
                       m_Y( 0 ),
                       m_Z( 0 ) { }

GantryPosition::GantryPosition( int x,
		    int y,
		    int z ) : m_X( x ),
				       m_Y( y ),
				       m_Z( z ) {
...
void GantryPosition::setX( int x ) {
  try {
    m_X = x;
...
void GantryPosition::setY( int y ) {
  try {
    m_Y = y;
...
void GantryPosition::setZ( int z ) {
  try {
    m_Z = z;
...
const char *GantryPosition::readObject( const char *str ) {
  try {
    /**
     * First the X number...
     */
...
void GantryPosition::writeObject( string& buffer ) {
  buffer += "    <position>\n"
            "      <x>";
...
double GantryPosition::distanceBetweenPoints( const GantryPosition& a, 
					const GantryPosition& b ) {
  return sqrt( ( a.getX() - b.getX() ) * ( a.getX() - b.getX() ) +
	       ( a.getY() - b.getY() ) * ( a.getY() - b.getY() ) +
	       ( a.getZ() - b.getZ() ) * ( a.getZ() - b.getZ() ) );
}

void GantryPosition::calculateIntermediatePosition( GantryPosition&       buffer,
					      double          fraction,
					      const GantryPosition& a,
					      const GantryPosition& b ) {
  try {
