...
void RealGantryRobot::getPosition( GantryPosition& positionBuffer ) const {
...
const GantryPosition& RealGantryRobot::getPosition() const {
...
void RealGantryRobot::moveToPosition( const GantryPosition& position,
				                              bool            waitFlag ) {
    bool failed;
    ...
    int      noOfTries = 0;
    GantryPosition currentPosition;
    do {
        ...
        if ( waitFlag == true ) {
            ...
            double currentDistance = GantryPosition::distanceBetweenPoints( position, currentPosition );
...
void RealGantryRobot::followSlewData( const SlewData& slewData, SlewUpdate *slewUpdate ) {
  try {
    ...
    for ( currentCoordinate = slewData.begin(); 
      ...
      GantryPosition currentPosition = getPosition();
