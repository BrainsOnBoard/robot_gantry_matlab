...
void SimulatedGantryRobot::getPosition( GantryPosition& positionBuffer ) const {
  try {
    if ( m_StartOfMoveTicks == 0 ) {
       ...
       */
      double distance = GantryPosition::distanceBetweenPoints( m_DesiredPosition,
							 m_StartPosition );
	...
	GantryPosition::calculateIntermediatePosition( positionBuffer,
...
const GantryPosition& SimulatedGantryRobot::getPosition() const {
...
void SimulatedGantryRobot::moveToPosition( const GantryPosition& position,
					   bool            waitFlag ) {
  m_DesiredPosition     = position;
  GantryPosition tempPosition = getPosition();
...
void SimulatedGantryRobot::followSlewData( const SlewData&, SlewUpdate * ) { }
