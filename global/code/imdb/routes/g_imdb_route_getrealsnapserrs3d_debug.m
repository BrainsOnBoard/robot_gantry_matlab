function [imxi,imyi,heads,whsn,err,nearest,dist,snx,sny,snth,errsel,p,fileexists,allwhsn,ridfs]=g_imdb_route_getrealsnapserrs3d_debug(shortwhd,arenafn,routenum,res,zht,useinfomax,forcegen)
if nargin < 5
    forcegen = false;
end
...
figdatfn = fullfile(g_dir_imdb_routes_figdata,sprintf('wrapped_g_imdb_route_geterrs_%s_%s_%03d_res%03d_z%d%s.mat',shortwhd,matfileremext(arenafn),routenum,res,zht,infomaxstr));
...
if fileexists && ~forcegen
    ...
    [snaps,clickis,snx,sny,snth]=g_imdb_route_getrealsnaps3d(arenafn,routenum,res);
